...
pub async fn refresh_cookie(
    ...
    let auth_info =
        refresh_cookie_by_uid_inner(bilibili, login_store, db, log_path, target_uid, false)
            .await?
...
pub async fn refresh_cookie_by_uid(
    ...
    let _guard = refresh_lock().lock().await;
    let auth_info =
        refresh_cookie_by_uid_inner(bilibili, login_store, db, log_path, user_id, false)
            .await?
            .ok_or_else(|| "刷新后登录信息无效".to_string())?;
    Ok(auth_info)
}
    ...
    for user_id in user_ids {
        ...
        match result {
            Ok(Some(_)) => {
                append_log(
                    log_path,
                    &format!("cookie_refresh_account_ok uid={}", user_id),
                );
            }
            Ok(None) => {
                append_log(
                    log_path,
                    &format!("cookie_refresh_account_skip uid={}", user_id),
                );
            }
            Err(err) => {
                append_log(
...
fn load_login_expire_time_by_uid(db: &Db, user_id: i64) -> Result<Option<DateTime<Utc>>, String> {
    db.with_conn(|conn| {
        let mut stmt =
            conn.prepare("SELECT expire_time FROM login_info WHERE user_id = ?1 LIMIT 1")?;
...
fn build_refreshed_login_data(login_data: &Value, cookie: String, refresh_token: String) -> Value {
