  ...
  match validate_baidu_login_http(
    ...
    Ok(http_info) => {
      let info = BaiduLoginInfo {
        ...
        username: http_info.username.clone().or_else(|| previous.username.clone()),
        login_type: previous.login_type,
        login_time: previous.login_time,
        last_check_time: Some(now),
      };
...
fn sync_baidu_cli_config(credential: &BaiduLoginCredential) -> Result<(), String> {
  ...
  if let Some(users) = root.get_mut("baidu_user_list").and_then(|value| value.as_array_mut()) {
    users.retain(|item| {
      let value = item
        .get("uid")
  ...
  user["accesstoken"] = user.get("accesstoken").cloned().unwrap_or_else(|| json!(""));
  user["workdir"] = json!("/");
...
fn build_baidu_api_cookie_header(credential: &BaiduLoginCredential) -> Result<String, String> {
  let cookie_minimal = |bduss: Option<String>, stoken: Option<String>| -> Result<Option<String>, String> {
...
fn fetch_baidu_remote_entries_via_http(
  ...
  if !status.is_success() {
    return Err(format!("请求百度网盘目录失败: HTTP {} {}", status.as_u16(), text));
  }
  let payload: Value =
    serde_json::from_str(&text).map_err(|err| format!("解析百度网盘目录响应失败: {} | {}", err, text))?;
  let errno = payload.get("errno").and_then(|value| value.as_i64()).unwrap_or(-1);
  if errno != 0 {
...
fn validate_baidu_login_http(
  ...
  } else if let Some(uid) = fallback_uid.map(|value| value.trim()).filter(|value| !value.is_empty()) {
    uid.to_string()
  } else {
    user_info.uid
  };
...
fn fetch_baidu_user_info_via_http(
  ...
  let errno = payload.get("errno").and_then(|value| value.as_i64()).unwrap_or(-1);
  if errno != 0 {
    return Err(format!("百度网盘用户信息读取失败: errno={} {}", errno, text));
  }
  let uid = payload
    .get("uk")
    .and_then(|value| value.as_i64())
