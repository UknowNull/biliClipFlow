...
impl LoginStore {
    ...
    pub fn save_login_info(
        ...
        let now = Utc::now();
        let expire_time =
            extract_expire_time(login_data).unwrap_or_else(|| now + Duration::hours(24));
    ...
    pub fn save_login_info_without_switching(
        ...
        if previous_primary_uid != saved_user_id {
            crate::account_store::set_primary_bilibili_uid(db, previous_primary_uid).map_err(
                |err| LoginStoreError::Io(std::io::Error::new(std::io::ErrorKind::Other, err)),
            )?;
        }
        ...
        Ok(records
            ...
            .map(
                |(
                    ...
                    let fallback_name = parsed_cookie_info.as_ref().and_then(|data| {
                        extract_string(data, &["nickname", "uname", "username", "name"])
                    });
    ...
    pub fn list_account_user_ids(&self, db: &Db) -> Result<Vec<i64>, LoginStoreError> {
        Ok(db.with_conn(|conn| {
            let mut stmt =
                conn.prepare("SELECT user_id FROM login_info ORDER BY login_time DESC")?;
    ...
    pub fn get_active_account_uid(&self, db: &Db) -> Result<Option<i64>, LoginStoreError> {
        crate::account_store::get_active_bilibili_uid(db)
            .map_err(|err| LoginStoreError::Io(std::io::Error::new(std::io::ErrorKind::Other, err)))
    }
    ...
    pub fn get_primary_account_uid(&self, db: &Db) -> Result<Option<i64>, LoginStoreError> {
        crate::account_store::get_primary_bilibili_uid(db)
            .map_err(|err| LoginStoreError::Io(std::io::Error::new(std::io::ErrorKind::Other, err)))
    }
    ...
    pub fn logout_by_uid(&self, db: &Db, user_id: i64) -> Result<(), LoginStoreError> {
        ...
        if primary_uid == Some(user_id) {
            ...
            crate::account_store::set_primary_bilibili_uid(db, next_primary_uid).map_err(
                |err| LoginStoreError::Io(std::io::Error::new(std::io::ErrorKind::Other, err)),
            )?;
        }
        Ok(())
    ...
    pub fn ensure_primary_account(
        &self,
        db: &Db,
        fallback_user_id: Option<i64>,
    ) -> Result<(), LoginStoreError> {
...
fn extract_refresh_token(data: &Value) -> Option<String> {
    extract_string(data, &["refresh_token"]).or_else(|| extract_url_param(data, "refresh_token"))
}
