...
#[tauri::command]
pub fn baidu_sync_settings(
    state: State<'_, AppState>,
) -> ApiResponse<baidu_sync::BaiduSyncSettings> {
...
#[tauri::command]
pub fn baidu_sync_status(state: State<'_, AppState>) -> ApiResponse<baidu_sync::BaiduLoginInfo> {
    ...
    window.on_window_event(move |event| {
        ...
        if let Some(sender) = sender {
            ...
            tauri::async_runtime::spawn(async move {
                let result = fetch_baidu_cookie_header(
                    &window_for_fetch,
                    &url_for_fetch,
                    &passport_for_fetch,
                )
            ...
            match result {
                ...
                summarize_cookie_names(&names)
                    ...
                    if has_stoken && cooldown_ticks == 0 {
                        ...
                        if missing_bduss_count >= 3
                            && jump_count < 2
                            && jump_count < jump_urls.len()
                        {
                            let target_url = jump_urls[jump_count];
                            missing_bduss_count = 0;
                            jump_count += 1;
                            cooldown_ticks = 3;
                            if let Err(err) = window_for_poll
                                .eval(&format!("window.location.replace('{}')", target_url))
                            {
                                append_log(
                                    app_log_for_poll.as_ref(),
                                    &format!(
                                        "baidu_login_jump_fail url={} err={}",
                                        target_url, err
                                    ),
                                );
                            } else {
...
fn build_cookie_header_with_flag(
    ...
    let has_auth = names
        .iter()
        .any(|name| name.to_uppercase().contains("BDUSS"));
    let header = names
        .iter()
        .map(|key| {
...
fn has_baidu_busy_output(output: &[String]) -> bool {
    output
        .iter()
        .any(|line| line.contains("50052") || line.contains("系统繁忙"))
}
