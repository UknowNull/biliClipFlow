...
pub async fn video_playurl(
    ...
    let params = vec![
        ...
        (
            "fnval".to_string(),
            fnval.unwrap_or_else(|| "4048".to_string()),
        ),
        (
            "fnver".to_string(),
            fnver.unwrap_or_else(|| "0".to_string()),
        ),
        (
            "fourk".to_string(),
            fourk.unwrap_or_else(|| "1".to_string()),
        ),
    ];
...
pub async fn bilibili_collections(
    ...
    if auth.is_none() {
        append_log(
            &state.app_log_path,
            &format!("collections_no_auth mid={}", mid),
        );
...
pub async fn bilibili_topics(
    ...
    for item in &raw_items {
        ...
        let show_activity_icon =
            parse_bool_field(item, &["show_activity_icon", "showActivityIcon"]);
...
fn load_auth(state: &State<'_, AppState>) -> Option<AuthInfo> {
    state
        .login_store
        .load_primary_auth_info(&state.db)
        .ok()
        .flatten()
}
