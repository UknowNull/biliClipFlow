...
use crate::utils::{
    append_log, apply_no_window, build_output_path, now_rfc3339, sanitize_filename,
};
use crate::AppState;
...
pub async fn download_video(
    ...
    let integration =
        payload.get("downloadRequest").is_some() || payload.get("downloadRequests").is_some();
    if integration {
            ...
            Ok(play_info) => {
                ...
                if let Some(value) = duration {
                    append_log(
                        ...
                        &format!(
                            "download_retry_duration task_id={} duration={}",
                            task_id, value
                        ),
                    );
                } else {
                    append_log(
                        &context.app_log_path,
                        &format!(
                            "download_retry_duration task_id={} duration=missing",
                            task_id
                        ),
                    );
                }
...
async fn requeue_download_record(context: &DownloadContext, record_id: i64) -> Result<(), String> {
    let record = context
        .db
        .with_conn(|conn| {
            conn.query_row(
...
pub fn recover_stale_downloads(state: &AppState) {
    ...
    let stale_ids = context
        .db
        .with_conn(|conn| {
            let mut stmt = conn.prepare("SELECT id FROM video_download WHERE status = 1")?;
        ...
        .is_ok()
    {
        append_log(
            &context.app_log_path,
            &format!(
                "download_recover_stale status=paused count={}",
                stale_ids.len()
            ),
        );
    }
...
async fn handle_integration_download(
    ...
    let normalized_baidu_sync_filename = crate::commands::submission::normalize_baidu_sync_filename(
...
fn download_path_conflict(context: &DownloadContext, output_path: &Path) -> Result<bool, String> {
    let path_value =
        to_stored_local_path(context.db.as_ref(), output_path.to_string_lossy().as_ref());
    ...
    let cid = match record.cid {
        ...
        None => {
            append_log(
                &context.app_log_path,
                &format!(
                    "download_schedule_invalid record_id={} reason=missing_cid",
                    record.id
                ),
            );
            let _ = update_download_status(&context, record.id, 3, 0);
...
async fn run_download_job(
    ...
    append_log(
        &context.app_log_path,
        &format!(
            "download_job_start record_id={} cid={}",
            record_id, part.cid
        ),
    );
    ...
    match result {
        Ok(()) => {
            ...
            append_log(
                &context.app_log_path,
                &format!(
                    "download_job_complete record_id={} status=completed",
                    record_id
                ),
            );
            let _ = refresh_integration_status(&context, record_id).await;
        ...
        Err(err) => {
            if is_resume_error(&err) {
                ...
                append_log(
                    &context.app_log_path,
                    &format!(
                        "download_job_complete record_id={} status=failed err={}",
                        record_id, err
                    ),
                );
            }
...
async fn fetch_baidu_remote_file_size_with_timeout(
    context: &DownloadContext,
    remote_path: &str,
) -> Result<u64, String> {
    ...
    let size_future = tauri::async_runtime::spawn_blocking(move || {
        baidu_sync::fetch_baidu_remote_file_size(db.as_ref(), &remote_path_for_task)
    });
    match timeout(
        Duration::from_secs(BAIDU_DOWNLOAD_SIZE_CHECK_TIMEOUT_SECS),
        size_future,
    )
    .await
    {
        Ok(Ok(result)) => result,
        Ok(Err(err)) => Err(format!("网盘文件大小检查线程失败: {}", err)),
        Err(_) => Err(format!(
            "网盘文件大小检查超时({}s): {}",
            BAIDU_DOWNLOAD_SIZE_CHECK_TIMEOUT_SECS, remote_path_owned
        )),
    }
}
...
async fn run_baidu_download_job(
    ...
    let total_size = match fetch_baidu_remote_file_size_with_timeout(&context, &remote_path).await {
        Ok(value) => value,
        Err(err) => {
            append_log(
                &context.app_log_path,
                &format!(
                    "baidu_download_size_check_fail record_id={} err={}",
                    record_id, err
                ),
            );
            0
    ...
    match download_result {
        Ok(path) => {
            if total_size > 0 {
                ...
                let _ =
                    update_download_bytes(&context, record_id, "baidu", total_size, current_size);
            }
            let _ = update_download_status(&context, record_id, 2, 100);
            clear_download_progress(&context, record_id);
            append_log(
                &context.app_log_path,
                &format!(
                    "baidu_download_job_complete record_id={} status=completed",
                    record_id
                ),
            );
            let _ = handle_baidu_restore_after_download(&context, record_id).await;
...
async fn merge_dash_stream_files(
    context: &DownloadContext,
    record_id: i64,
    ...
    args.push(output_path_string.to_string());

    if let Err(err) = run_ffmpeg_job(
        context,
        record_id,
        false,
        duration_ms,
    ...
    let (video_duration, audio_duration) = match probe_stream_durations(output_path) {
        Ok(value) => value,
        Err(err) => {
            append_log(
                &context.app_log_path,
    ...
    log_ffprobe_av_timing(&context.app_log_path, record_id, log_source, output_path);

    if !is_video_complete(video_duration, audio_duration, expected_duration_seconds) {
        append_log(
            &context.app_log_path,
            &format!(
        ...
        let _ = std::fs::remove_file(output_path);
        return Err("Video stream too short".to_string());
    }
    if !is_audio_complete(video_duration, audio_duration) {
        ...
        let _ = std::fs::remove_file(output_path);
        return Err("Audio stream too short".to_string());
    }
...
async fn download_part(
    ...
    if format == "dash" && !has_dash && has_durl {
        append_log(
            &context.app_log_path,
            &format!(
                "playurl_format_fallback record_id={} from=dash to=mp4",
                record_id
            ),
        );
    ...
    match content.as_str() {
        "video_only" => {
            let video_candidates = select_video_candidates(
                dash,
                config.resolution.as_deref(),
                        ...
                        if video_result.is_err() || audio_result.is_err() {
                            ...
                            if can_resume_video && can_resume_audio {
                                append_log(
                                    &context.app_log_path,
                                    &format!(
                                        "aria2c_resume_merge_try record_id={} video={} audio={}",
                                        record_id,
                                ...
                                .await
                                {
                                    ...
                                    Err(err) => {
                                        append_log(
                                            &context.app_log_path,
                                            &format!(
                                                "aria2c_resume_merge_fail record_id={} err={}",
                                                record_id, err
                                            ),
                                        );
                                    }
                        ...
                        record_id, video_duration, audio_duration
                                    ...
                                    Err(err) => {
                                        append_log(
                                            &context.app_log_path,
                                            &format!(
                                                "ffprobe_check_fail record_id={} err={}",
                                                record_id, err
                                            ),
                                        );
                                        let _ = std::fs::remove_file(&output_path);
    ...
    for (index, url) in urls.iter().enumerate() {
        if index > 0 {
            append_log(
                &context.app_log_path,
                &format!(
                    "ffmpeg_url_retry record_id={} index={}",
                    record_id,
                    index + 1
                ),
            );
                ...
                Ok(None) => {
                    if !logged_no_status {
                        ...
                        append_log(
                            app_log_path,
                            &format!(
                                "aria2c_rpc_no_status record_id={} key={}",
                                record_id, progress_key
                            ),
                        );
                    }
                }
...
async fn fetch_play_info(
    ...
    let qn = config.resolution.clone().unwrap_or_else(|| {
        if is_logged_in {
            "127".to_string()
        } else {
            "64".to_string()
...
fn choose_target_codec(
    ...
    if let Some(codec) = codec {
        if filtered
            .iter()
            .any(|candidate| candidate_codec_matches(candidate, codec))
        {
    ...
    filtered
        .iter()
        .find_map(|candidate| candidate.codec.clone())
}

fn select_audio_candidates(dash: &Value, block_pcdn: bool) -> Result<Vec<StreamCandidate>, String> {
    let audios = dash
        .get("audio")
        .and_then(|value| value.as_array())
...
fn select_video_candidates(
    ...
    candidates.sort_by(|a, b| {
        let a_res = target_resolution
            .map(|resolution| a.id == Some(resolution))
            .unwrap_or(false);
        let b_res = target_resolution
            .map(|resolution| b.id == Some(resolution))
            .unwrap_or(false);
...
fn log_ffprobe_av_timing(app_log_path: &Path, record_id: i64, source: &str, path: &Path) {
    match probe_stream_timing(path) {
        Ok(timing) => {
...
fn build_ffmpeg_headers(context: &DownloadContext) -> Option<String> {
    ...
    headers
        .push_str("User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36\r\n");
...
fn reset_download_record_progress(context: &DownloadContext, record_id: i64) -> Result<(), String> {
    let now = now_rfc3339();
    context
    .db
    .with_conn(|conn| {
...
fn reset_download_progress_bytes(context: &DownloadContext, record_id: i64) -> Result<(), String> {
    let now = now_rfc3339();
    context
        .db
        .with_conn(|conn| {
...
fn reset_integrated_submission_status(
    ...
    let submission_status =
        load_submission_status(context, &submission_task_id).unwrap_or_default();
