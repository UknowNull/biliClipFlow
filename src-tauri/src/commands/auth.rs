...
pub async fn auth_qrcode_generate(
    ...
    let url = format!(
        "{}{}",
        state.bilibili.passport_base_url(),
        QR_CODE_GENERATE_PATH
    );
...
#[tauri::command]
pub fn auth_accounts_list(state: State<'_, AppState>) -> ApiResponse<Vec<BilibiliAccountSummary>> {
...
#[tauri::command]
pub fn account_binding_list(state: State<'_, AppState>) -> ApiResponse<Vec<AccountBindingRecord>> {
...
pub async fn auth_perform_qrcode_login(
    ...
    tauri::async_runtime::spawn(async move {
        if let Ok(qr_data) = bilibili
            .get_json(
                &format!("{}{}", bilibili.passport_base_url(), QR_CODE_GENERATE_PATH),
                &[],
                None,
                false,
            )
            .await
        {
            ...
            if let Some(qrcode_key) = qrcode_key {
                ...
                while attempts < 30 {
                    ...
                    if let Ok(result) = poll_qrcode_once_inner(
                        &bilibili,
                        &login_store,
                        &db,
                        &qrcode_key,
...
async fn build_auth_status(state: &AppState) -> Result<HashMap<String, Value>, String> {
    ...
    data.insert(
        "accounts".to_string(),
        serde_json::to_value(&accounts).unwrap_or_else(|_| Value::Array(Vec::new())),
    );
    data.insert(
...
async fn fetch_profile(bilibili: &BilibiliClient, cookie: &str) -> Result<Value, String> {
    ...
    let stat = bilibili
        .get_json(&stat_url, &[], Some(&auth), false)
        .await
        .ok();
    ...
    let user_info = if let Some(mid) = mid {
        ...
        bilibili
            .get_json(&user_url, &params, Some(&auth), true)
            .await
            .ok()
    } else {
        None
...
async fn exchange_cookie_from_url(
    url: &str,
    log_path: Option<&Path>,
